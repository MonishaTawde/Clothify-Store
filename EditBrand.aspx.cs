...
namespace NyraShopping.Root
{
    public partial class EditBrand : System.Web.UI.Page
    {
        ...
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["NyraShoppingDB"].ConnectionString);
        ...
        protected void BtnDeleteBrand_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["NyraShoppingDB"].ConnectionString);
            if (con.State == ConnectionState.Closed) { con.Open(); }
            SqlCommand cmd = new SqlCommand("DELETE FROM tblBrands WHERE UPPER(Name) = UPPER(@Name)", con);
            cmd.Parameters.AddWithValue("@Name", txtUpdateBrandName.Text);
            cmd.ExecuteNonQuery();
            con.Close();
