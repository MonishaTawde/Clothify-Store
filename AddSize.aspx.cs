...
namespace NyraShopping.Root
{
    public partial class AddSize : System.Web.UI.Page
    {
        ...
        private void BindrptrSize()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT A.SizeID, A.SizeName, B.CatName, C.Name, D.SubCatName, E.GenderName FROM tblSizes A INNER JOIN tblCategory B ON B.CatID = A.CategoryID INNER JOIN tblBrands C ON C.BrandID = A.BrandID INNER JOIN tblSubCategory D ON D.SubCatID = A.SubCategoryID INNER JOIN tblGender E ON E.GenderID = A.GenderID", con))
                {
                    con.Open();
